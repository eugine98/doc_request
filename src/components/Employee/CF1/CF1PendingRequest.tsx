...
const fetch_pending_cf1 = async () => {
    try {
        ...
        const response = await axios.post(`${API_SERVER_URL}/Api/fetch_pending_cf1`, formdata);
...
//  }, [pendingRequest])
  ...
  const confirmExitWsaving = () => {
    ...
    seteditDateHired(pendingRequest.data.c_date_hired)
    ...
    if (dateHiredRef.current) dateHiredRef.current.value = pendingRequest.data.c_date_hired;
  ...
  const closeEdit = () => {
    if(pendingRequest.data.c_purpose != editReason.trim()){
      setExitWsaving(true);
    }else{
      setEditReason(pendingRequest.data.c_purpose)
      seteditDateHired(pendingRequest.data.c_date_hired)
  ...
  const confirmDelete = async () => {
    try {
      ...
      await axios.post(`${API_SERVER_URL}/Api/delete_cf1_req`, formdata);
      fetch_pending_cf1();
      refetchPendingRequest();
      toast.success('', {
        className: 'my-classname',
  ...
  const savingChangesValidation = () => {
    if(editReason.trim() != ""){
      if(pendingRequest.data.c_emp_status == 'active'){
        ...
        if(pendingRequest.data.c_purpose != editReason.trim() || pendingRequest.data.c_date_resigned != editDateResigned || pendingRequest.data.c_date_hired != editDateHired){
          setConfirmChanges(true)
        }else{
          toast.warning('', {
            className: 'my-classname',
  ...
  const save_Changes = async () => {
    //alert(pendingRequest.data.c_id)
    try {
      const formdata = new FormData();
      formdata.append("id", pendingRequest.data.c_id);
      formdata.append("date_hired", editDateHired);
      formdata.append("date_resigned", editDateResigned);
      formdata.append("purpose", editReason);
      const response = await axios.post(`${API_SERVER_URL}/Api/update_cf1_req`, formdata);
             ...
             >
                ...
                {pendingRequest.data.c_emp_status == 'active' ? ( 
                    ...
                    <div>
                      ...
                      <input 
                       disabled={!editMode} 
                         type="date" id="date_hired" className='mt-1 block w-full border rounded-md p-2' 
                         defaultValue={pendingRequest.data.c_date_hired}
                ...
                <div className='absolute full p-10 md-p-0'>
                    ...
                    <ol className="relative text-gray-500 border-s border-gray-100 dark:border-gray-700 dark:text-gray-400 w-full">    
                        ...
                        <li className="mb-5 ms-6">
                            ...
                            <ul style={{ listStyleType: 'disc', paddingLeft: '25px' }} className='text-[0.8rem] '>
                              <li className={` ${pendingRequest.data.c_req_status > 3 && 'text-green-700'}`}>Accounting Staff will process the CF1.</li>
                              <li className={` ${pendingRequest.data.c_req_status > 4 && 'text-green-700'}`}>Accounting Staff will give the CF1 to their Accounting Manager for her sign.</li>
                              <li className={` ${pendingRequest.data.c_req_status > 5 && 'text-green-700'}`}>Accounting Staff will transmitted back the Signed CF1 to Hr Staff</li>
                            </ul>
                          </li>
                        {/*3rd step*/}
                        <li className="mb-5 ms-6">
                        {pendingRequest.data.c_req_status <= 6 ? (
                            ...
                            <ul style={{ listStyleType: 'disc', paddingLeft: '25px' }} className='text-[0.8rem] '>
                              <li className={` ${pendingRequest.data.c_req_status == 7 && 'animate-pulse font-medium text-orange-600'}`}>Claim your Document at HR Office</li>
                            </ul>
                        </li>
                    </ol>
                    </div>
...
export default CF1PendingRequest
