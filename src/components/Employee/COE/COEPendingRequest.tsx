...
const fetch_pending_coe = async () => {
    ...
    try {
        ...
        const response = await axios.post(`${API_SERVER_URL}/Api/fetch_pending_coe`, formdata);
...
// }, [isLoading])
  ...
  const savingChangesValidation = () => {
    if(editReason.trim() != ""){
      if(pendingRequest.data.c_emp_status == 'active'){
        ...
        if(pendingRequest.data.c_purpose != editReason.trim() || pendingRequest.data.c_date_resigned != editDateResigned){
          setConfirmChanges(true)
        }else{
          toast.warning('', {
            className: 'my-classname',
  ...
  const save_Changes = async () => {
    try {
      ...
      await axios.post(`${API_SERVER_URL}/Api/update_coe_req`, formdata);
      fetch_pending_coe();
      refetchPendingRequest();
      toast.success('', {
        className: 'my-classname',
                ...
                <div className='absolute full p-10 md-p-0 '>
                    ...
                    <ol className="relative text-gray-500 border-s border-gray-100 dark:border-gray-700 dark:text-gray-400 w-full">    
                        ...
                        <li className="mb-6 ms-6">    
                            {pendingRequest.data.c_req_status <=4 ? (
                            ...
                            <ul style={{ listStyleType: 'disc', paddingLeft: '25px' }} className='text-[0.8rem] '>
                              <li className={` ${pendingRequest.data.c_req_status > 0 && 'text-green-700'}`}>Hr Staff1 to validate the request form</li>
                              <li className={` ${pendingRequest.data.c_req_status > 1 && 'text-green-700'}`}>Hr Staff1 to submit the request form to the HR Manager for her sign</li>
                              <li className={` ${pendingRequest.data.c_req_status > 2 && 'text-green-700'}`}>Hr Staff1 will give the signed request form to Hr staff2</li>
                              <li className={` ${pendingRequest.data.c_req_status > 3 && 'text-green-700'}`}>Hr Staff2 will do the Coe and print.</li>
                              <li className={` ${pendingRequest.data.c_req_status > 4 && 'text-green-700'}`}>Hr Staff2 will give the printed COE to Hr Manager for her sign.</li>
                            </ul>
                        </li>
                        {/*2nd step*/}
                        <li className="mb-6 ms-6">
                        {pendingRequest.data.c_req_status <= 5 ? (
                            ...
                            <ul style={{ listStyleType: 'disc', paddingLeft: '25px' }} className='text-[0.8rem] '>
                              <li className={` ${pendingRequest.data.c_req_status == 6 && 'animate-pulse font-medium text-orange-600'}`}>Claim and sign the receiving copy.</li>
                            </ul>
                        </li>
                    </ol>
                    </div>
...
export default COEPendingRequest
