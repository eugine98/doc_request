...
function ITRPendingRequest() {
  ...
  const fetch_pending_itr = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_SERVER_URL}/Api/fetch_pending_itr`,
        formdata
      );
  ...
  const confirmExitWsaving = () => {
    ...
    if (reasonRef.current)
      reasonRef.current.value = pendingRequest.data.i_reason_request;
    if (dateResignedRef.current)
      dateResignedRef.current.value = pendingRequest.data.i_date_resigned;
    if (ITRYearRef.current)
      ITRYearRef.current.value = pendingRequest.data.i_itr_year;
    setEditMode(false);
  };
      ...
      toast.warning("", {
        ...
        description: (
          <div className="font-semibold">
            Action invalid: form already validated.
          </div>
        ),
        duration: 2500,
  ...
  const closeEdit = () => {
    if (
      pendingRequest.data.i_reason_request != editReason.trim() ||
      pendingRequest.data.i_itr_year != editITRYear
    ) {
      setExitWsaving(true);
    } else {
      setEditReason(pendingRequest.data.i_reason_request);
      ...
      if (pendingRequest.data.i_emp_status == "active") {
        if (
          ...
          toast.warning("", {
            className: "my-classname",
            description: (
              <div className="font-semibold">No changes were made!</div>
            ),
            duration: 2500,
            icon: <TriangleAlert className="h-5 w-5 mt-0.5" />,
          });
        ...
        if (
          pendingRequest.data.i_reason_request != editReason.trim() ||
          pendingRequest.data.i_itr_year != editITRYear ||
          pendingRequest.data.i_date_resigned != editDateResigned
        ) {
          setConfirmChanges(true);
        } else {
          toast.warning("", {
  ...
  return (
    <>
      ...
      <div>
        <AlertDialog open={confirmChanges} onOpenChange={setConfirmChanges}>
          <AlertDialogContent className="w-96">
            ...
            <AlertDialogFooter>
              <AlertDialogCancel className="h-8">No</AlertDialogCancel>
              <AlertDialogAction onClick={() => save_Changes()} className="h-8">
                Yes
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
      ...
      <div
        className={`flex items-center justify-center w-full bg-white dark:bg-gray-800 ${
          showLoader && "hidden"
        }`}
      >
        {!isLoading &&
        pendingRequest.code == 1 &&
        pendingRequest.data.i_req_status != undefined ? (
          <div
            className="w-full md:flex md:justify-between lg:pl-10 lg:pr-10 pl-5 pr-5"
            style={{
              background:
          ...
          >
            <div
              ...
              className={`relative md:w-1/2 w-full rounded-sm ${
                editMode && "bg-blue-200 mt-2"
              }`}
            >
              <div className="w-full absolute flex justify-end">
                <div
                  className={`w-full flex justify-between p-3 pb-0 ${
                    !editMode && "hidden"
                  }`}
                >
                  <button
                    type="submit"
                    className={`rounded-sm h-5 w-5  bg-red-600 flex justify-center text-white hover:bg-red-500 ${
                      !editMode && "hidden"
                    }`}
                    onClick={() => closeEdit()}
                  >
                  ...
                  <button
                    type="submit"
                    className={`h-7 w-16 rounded-sm bg-blue-600 flex justify-center text-white ${
                      !editMode && "hidden"
                    }`}
                ...
                <div className="fixed top-28 mr-4">
                  <DropdownMenu>
                    <DropdownMenuTrigger
                      disabled={pendingRequest.data.i_req_status == undefined}
                    >
              ...
              <form
                className="w-full p-4 space-y-4 mt-1.5 pt-10 sm-pt-4 rounded shadow-md text-gray-700 text-[0.63rem] font-semibold"
              ...
              >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    required
                    ref={reasonRef}
                    className="mt-1 block w-full border rounded-md p-2 h-16 "
                    disabled={!editMode}
            ...
            <div
              className={`relative md:ms-12 md:w-1/2 w-full pt-5 ${
                showLoader && "hidden"
              }`}
            >
              <div className="absolute w-full pt-5 p-10 md-p-0">
                <div className=" md:fixed md:top-32">
                  <ul className="flex ms-6 mb-10 space-x-8 text-xs">
                    <li className="flex">
                      <p>Undone</p>
                      <CircleX className="ms-1 h-4 w-4 text-red-600" />
                    </li>
                    <li className="flex">
                      <p>Ongoing</p>{" "}
                      <CircleEllipsis className="ms-1 h-4 w-4 text-blue-600" />
                    </li>
                    <li className="flex">
                      <p>Done</p>
                      <CircleCheck className="ms-1 h-4 w-4 text-green-600" />
                    </li>
                  </ul>
                  <ol className="relative text-gray-500 border-s border-gray-100 dark:border-gray-700 dark:text-gray-400 w-full">
                    ...
                    <li className="mb-5 ms-6">
                      {pendingRequest.data.i_req_status <= 2 ? (
                        <CircleEllipsis
                          strokeWidth={1.3}
                          className="absolute flex items-center p-0 justify-center w-5 h-5 -start-2.5 bg-gray-100 rounded-full text-blue-600  dark:text-gray-400"
                        />
                      ) : (
                        pendingRequest.data.i_req_status > 2 && (
                          <CircleCheck
                            strokeWidth={1.3}
                            className="absolute flex items-center justify-center w-5 h-5 -start-2.5 bg-green-200 rounded-full text-green-600  dark:text-gray-400"
                          />
                        )
                      )}
                      ...
                      >
                        <li
                          className={` ${
                            pendingRequest.data.i_req_status > 0 &&
                            "text-green-700"
                          }`}
                        >
                    ...
                    <li className="mb-5 ms-6">
                      {pendingRequest.data.i_req_status <= 2 ? (
                        <CircleX
                          strokeWidth={1.3}
                          className="absolute flex items-center p-0 justify-center w-5 h-5 -start-2.5 bg-gray-100 rounded-full text-red-600  dark:text-gray-400"
                        />
                      ...
                      >
                        <li
                          className={` ${
                            pendingRequest.data.i_req_status > 3 &&
                            "text-green-700"
                          }`}
                        >
                    ...
                    <li className="mb-5 ms-6">
                      ...
                      >
                        <li
                          className={` ${
                            pendingRequest.data.i_req_status == 7 &&
                            "animate-pulse font-medium text-orange-600"
                          }`}
                        >
